...
contract DSCEngine is ReentrancyGuard {
    ...
    constructor(address[] memory tokenAddresses, address[] memory priceFeedAddresses, address dscAddress) {
        ...
        // For examole ETH / USD, BTC / USD, MKR / USD, etc
    ...
    /*
     * @notice tokenCollateralAddress The address of the token to deposit as collateral
    ...
    * This function burn DSC and redeems collateral in one transaction
    */
    function redeemCollateralForDsc(address tokenCollateralAddress, uint256 amountCollateral, uint256 amountDscToBurn) external  {
        burnDsc(amountDscToBurn);
        redeemCollateral(tokenCollateralAddress, amountCollateral);
        // redeemCollateral will revert if they don't have enough collateral i.e it already checks for health factor
    }
    ...
    * @notice A known bug would be if the protocol were 100% or less collateralized, then we wouldn't be able to incentivize the liquidatrors.
    ...
    */
    function liquidate(address collateral, address user, uint256 debtToCover) external moreThanZero(debtToCover) nonReentrant {
        ...
        // uint256 collateralBalance = s_collateralDeposited[user][collateral];
        // uint256 totalCollateralToRedeem = tokenAmountFromDebtCovered + bonusCollateral;

        // // 🛡️ Prevent underflow if liquidation bonus exceeds user collateral
        // if (totalCollateralToRedeem > collateralBalance) {
        //   totalCollateralToRedeem = collateralBalance;
        // }


        _redeemCollateral(user, totalCollateralToRedeem,msg.sender, collateral);
    ...
    /*
    * @dev Low-level internal functions, do not call unless the function calling it is checking for health factor being broken
     */
    ...
    */
    function _healthFactor(address user) private view returns (uint256) {
        ...
        if (totalDscMinted == 0) {
            // No debt, so health factor is infinite i.e health factor is still positive
        ...
        // $150 ETH / 100 DSC = 1.5
        // 150 * 50 = 7500 / 100 = (75 / 100) < 1 (liquidation)

        // $1000 ETH / 100 DSC
        // 1000 * 50 = 50,000 / 100 = (500 / 100) = 5 > 1 (not liquidation)
